...
export default function BetriebsdatenPage() {
  const navigate = useNavigate();
  const [records, setRecords] = useState<Betriebsdaten[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Betriebsdaten | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getBetriebsdaten());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Betriebsdaten['fields']) {
    await LivingAppsService.createBetriebsdatenEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Betriebsdaten['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateBetriebsdatenEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteBetriebsdatenEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Betriebsdaten"
      subtitle={`${records.length} Betriebsdaten im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Betriebsdaten suchen..."
          value={search}
          ...
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('messgroesse')}>
                <span className="inline-flex items-center gap-1">
                  Messgröße
                  {sortKey === 'messgroesse' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('wert')}>
                <span className="inline-flex items-center gap-1">
                  Messwert
                  {sortKey === 'wert' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
