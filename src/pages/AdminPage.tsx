...
import { BetriebsdatenViewDialog } from '@/components/dialogs/BetriebsdatenViewDialog';
...
const ENTITY_TABS = [
  { key: 'betriebsdaten', label: 'Betriebsdaten', pascal: 'Betriebsdaten' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('betriebsdaten');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'betriebsdaten': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'betriebsdaten': return (data as any).betriebsdaten as Betriebsdaten[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'betriebsdaten': return BETRIEBSDATEN_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'betriebsdaten': return {
        create: (fields: any) => LivingAppsService.createBetriebsdatenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateBetriebsdatenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteBetriebsdatenEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'betriebsdaten' || dialogState?.entity === 'betriebsdaten') && (
        <BetriebsdatenDialog
          open={createEntity === 'betriebsdaten' || dialogState?.entity === 'betriebsdaten'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'betriebsdaten' ? handleUpdate : (fields: any) => handleCreate('betriebsdaten', fields)}
          defaultValues={dialogState?.entity === 'betriebsdaten' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Betriebsdaten']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Betriebsdaten']}
        />
      )}
      {viewState?.entity === 'betriebsdaten' && (
        <BetriebsdatenViewDialog
          open={viewState?.entity === 'betriebsdaten'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'betriebsdaten', record: r }); }}
        />
      )}
