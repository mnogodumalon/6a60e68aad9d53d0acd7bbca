...
interface BetriebsdatenViewDialogProps {
  open: boolean;
  onClose: () => void;
  record: Betriebsdaten | null;
  onEdit: (record: Betriebsdaten) => void;
}

export function BetriebsdatenViewDialog({ open, onClose, record, onEdit }: BetriebsdatenViewDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Betriebsdaten anzeigen</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm">{record.fields.messgroesse ?? '—'}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Messwert</Label>
            <p className="text-sm">{record.fields.wert ?? '—'}</p>
          </div>
          <div className="space-y-1">
          ...
          <div className="pt-2 border-t border-border">
            <AttachmentsSection appId={APP_IDS.BETRIEBSDATEN} recordId={record.record_id} readOnly />
          </div>
        </div>
      </DialogContent>
    </Dialog>
