...
interface BetriebsdatenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Betriebsdaten['fields']) => Promise<void>;
...
export function BetriebsdatenDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: BetriebsdatenDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Betriebsdaten['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'betriebsdaten');
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Betriebsdaten['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.BETRIEBSDATEN} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
